...
public class HUSHiding
{
    ...
    /// </summary>
    readonly int BUFFERS_SIZE = 30000; //2000 cũ của Tai ko chay dc kosarac 990k
        ...
        foreach (KeyValuePair<int, float> entry in mapItemSWU.SetOfKeyValuePairs())
        {
            if (entry.Value >= minUtility)
            {
                        ...
                        for (itemset = qmatrix.MatrixItemRemainingUtility[row].Length - 1; itemset >= 0; itemset--)
                        {
                            ...
                            if (utility > 0)
                            {
                                ...
                                if (peuts > maxpeuts)
                                    maxpeuts = peuts;
                                if (uList == null)
                                    uList = new UtilityList(tidList, utility, remaining);
                                else
                                {
                                    uList = new UtilityList(tidList, utility, remaining, uList);
                                }
                            }
                        }
                ...
                if (totalUtility >= minUtility)
                {
                    ...
                    _huspCount++;
                    HUS_Hiding(items, utilityChains, totalUtility, uid);
                }
        ...
        foreach (KeyValuePair<int, float> entry in iList.SetOfKeyValuePairs())
        {
            ...
            if (entry.Value >= minUtility)
            {
                        ...
                        if (row >= 0)
                        {
                            do
                            {
                                ...
                                if (utility > 0)
                                {
                                    ...
                                    if (peuts > maxpeuts)
                                        maxpeuts = peuts;
                                    UtilityList newList = new UtilityList(tidList, acu, remaining);
                                    tmpList.Add(newList);
                                }
                                utilityList = utilityList.Link;
                            } while (utilityList != null);
                ...
                if (totalUtility >= minUtility)
                {
                    ...
                    _huspCount++;
                    HUS_Hiding(items, utilityChainIConcatenation, totalUtility, uid);
                }
        ...
        foreach (KeyValuePair<int, float> entry in sList.SetOfKeyValuePairs())
        {
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (totalUtility >= minUtility)
                {
                    ...
                    _huspCount++;
                    HUS_Hiding(items, utilityChainSConcatenation, totalUtility, uid);
                }
    ...
    public void runAlgorithm(Dictionary<int, QMatrixHUSSpan> database, string uid)
    {
        ...
        // calculate time at begin not here
        //startTimestamp = DateTimeHelperClass.CurrentUnixTimeMillis();
    ...
    public bool _willModifiedData = false;
    public float _dbDelta { get; private set; }
    public double _hidingTotalTime { get; private set; }

    public void HUS_Hiding(int[] items, IList<UtilityChain> utilityChains, float utility, string uid)
    {
        var start = DateTime.Now;
        Dictionary<int, Dictionary<string, string>> modifyDic = new Dictionary<int, Dictionary<string, string>>();
        ...
        while (diff > 0)
        {
            ...
            foreach (UtilityChain utility1Chain in utilityChains)
            {
                if (utility1Chain.UtilityList != null)
                {
                    ...
                    do
                    {
                        if (utilityList.ACU == utility1Chain.MaxUtility)
                        {
                            tidList = utilityList.TID;
                            break;
                        }
                        else
                            utilityList = utilityList.Link;
            ...
            float anpha = (numberDecreasedItems * externalData[itemMax]) / uMax;
            //Second scanning to change quantity
            for (int k = 0; k < utilityChains.Count; k++)
            {
                if (utilityChains[k].UtilityList != null)
                {
                    float reducingSum = 0;
                    QMatrixHUSSpan qMatrix = database[utilityChains[k].SeqID];
                    // Get the utility list in the utility chain 
                    UtilityList utilityList = utilityChains[k].UtilityList;
                    List<int> tidList = new List<int>();
                    do
                    {
                        if (utilityList.ACU == utilityChains[k].MaxUtility)
                        {
                            tidList = utilityList.TID;
                            break;
                        }
                    ...
                    if (tidList.Count() > 0)
                    {
                        int itemsetIndex = tidList[indexMax]; //column
                        ...
                        else
                        {
                            ...
                            /*diff -= (qMatrix.MatrixItemUtility[row][itemsetIndex] - 1);
                            reducingSum += (qMatrix.MatrixItemUtility[row][itemsetIndex] - 1);
                            qMatrix.MatrixItemUtility[row][itemsetIndex] = 1;*/
