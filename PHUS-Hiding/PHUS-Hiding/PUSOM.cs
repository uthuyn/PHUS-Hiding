...
public class PUSOM
{
    ...
    private void husspanFirstTime(int[] prefix, int prefixLength, IList<QMatrixHUSSpan> database, float minPer, float maxPer, float minAvg, float maxAvg)
    {
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (checkPeriodicByIDList(seqIdList, maxPer))
                {
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        {
                            husspan(prefix, 1, utilityChains, 1, minPer, maxPer, minAvg, maxAvg);
                        }
                    }
                }
            }
    ...
    private void husspan(int[] prefix, int prefixLength, UtilityChainList utilityChains, int itemCount, float minPer, float maxPer, float minAvg, float maxAvg)
    {
        // =======================  I-Extension  ===========================/
            ...
            if (entry.Value >= minUtility)
            {
                ...
                for (int y = 0; y < listOfUtilityChain.Count; y++)
                {
                    ...
                    if (tmpList.Count > 0)
                    {
                        ...
                        finalList = tmpList[0];
                        exUtility1Chain = new UtilityChain(qmatrix.SeqID, maxpeuts, maxUtility, finalList); //new UtilityChain(maxpeuts, finalList);
                ...
                if (checkPeriodicByIDList(seqIdListIConcatenation, maxPer))
                {
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        {
                            husspan(prefix, prefixLength + 1, utilityChainsIConcatenation, itemCount + 1, minPer, maxPer, minAvg, maxAvg);
                        }
                    }
                }
            }
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (checkPeriodicByIDList(seqIdListSConcatenation, maxPer))
                {
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        if (itemCount + 1 < maxPatternLength)
                        {
                            husspan(prefix, prefixLength + 2, utilityChainsSConcatenation, itemCount + 1, minPer, maxPer, minAvg, maxAvg);
                        }
                    }
                }
            }
    ...
    public void runAlgorithm(IList<QMatrixHUSSpan> database, string output, float minPer, float maxPer, float minAvg, float maxAvg)
    {
        ...
        husspanFirstTime(patternBuffer, 0, database, minPer, maxPer, minAvg, maxAvg);
